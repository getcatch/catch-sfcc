 ...
 *
 * @transactional
 * @param {Object} stage - Catch checkout stage stage = 'create_order' || 'place_order'
...
function start(stage) {
    ...
    if (!stage || stage.stage === 'create_order') {
        var cart = Cart.get();
        ...
        // ---  Catch code start ---
        var catchIntegrationType = (stage && stage.stage === 'create_order')
            ? catchHelper.getCheckoutIntegrationType(cart) : null;
        // --- Catch code end ---
        ...
        if (stage && stage.stage === 'create_order') {
            ...
            return {
                ...
                catchCheckoutId: catchCheckoutResult.data.id,
    ...
    if (stage && stage.stage === 'place_order') {
        order = stage.order;
    }
    ...
    // --- Catch code start ----
    if (stage && stage.stage === 'place_order') {
        if (order.getPaymentInstruments('Catch').length === 0) {
            return {
                error: true,
                PlaceOrderError: new Status(Status.ERROR, 'confirm.error.technical')
            };
        ...
        if (order.custom.catch_integrationType === catchHelper.VIRTUAL_CARD_CHECKOUT) {
            // --- Handle Catch Virtual Card Checkout ---
            if (!catchCheckoutHelper.handleCatchVirtualCard(stage.virtualCard, order)) {
                return {
                    error: true,
                    PlaceOrderError: new Status(Status.ERROR, 'confirm.error.technical')
                };
            ...
            if (!catchPurchaseResult) {
                return {
                    error: true,
                    PlaceOrderError: new Status(Status.ERROR, 'error.purchase.id')
                };
            }
